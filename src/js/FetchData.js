...
// FETCH FROM API
export async function FetchData(containerSection, URL) {
  ...
  data.results.forEach((element) => {
    ...
    movieCard.innerHTML = `
      <img class="image" src="${request.imgpath + element.poster_path}" alt="">
      <div class="content">
    ...
    movieCard.addEventListener("click", (e) => {
      if (e.target.parentNode.className.includes("content__box-icon-trailer")) {
        watchTrailer(element.id);
      }
    });
    containerSection.appendChild(movieCard);
  });
}
